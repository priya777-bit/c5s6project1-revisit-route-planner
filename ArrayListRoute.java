...
public class ArrayListRoute
{
    ...
    public void showAllFlights(String source,String destination)
    {
        ...
        if(source1.isPresent())
        {
            ...
            {
                Optional<Route> source2 = routeInfo.stream().filter((p->p.getSource().equalsIgnoreCase(r.getSource()))).findAny();

                if(source2.isPresent())
                {
                    routeInfo.stream().filter((p->p.getSource().equalsIgnoreCase(r.getSource())));
                    ...
                    for(Route r1:routeInfo)
                    {
                        ...
                        {
                            routeInfo.stream().filter(p->(p.tempdestination.equalsIgnoreCase(r1.getSource()) && (p.getDestination().equalsIgnoreCase(r1.getDestination()))));
