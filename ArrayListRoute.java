...
public class ArrayListRoute
{
    ...
    public void showAllFlights(String source,String destination)
    {
        ...
        if(source1.isPresent())
        {
            ...
            {
                Optional<Route> source2 = routeInfo.stream().filter((p->this.source.equalsIgnoreCase(r.getSource()))).findAny();

                if(source2.isPresent())
                {
                    routeInfo.stream().filter((p->this.source.equalsIgnoreCase(r.getSource))).forEach(display);
                    tempdestination=r.getDestination();

                    for(Route r1:routeInfo)
                    {
                        Optional<Route> source3 = routeInfo.stream().filter(p->(this.tempdestination.equalsIgnoreCase(r1.getSource()) && (this.destination.equalsIgnoreCase(r1.getDestination())))).findAny();
                        if(source3.isPresent())
                        {
                            routeInfo.stream().filter(p->(this.tempdestination.equalsIgnoreCase(r1.getSource()) && (this.destination.equalsIgnoreCase(r1.getDestination())))).forEach(display);
